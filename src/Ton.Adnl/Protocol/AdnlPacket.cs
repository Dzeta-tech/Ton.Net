...
public sealed class AdnlPacket
{
    ...
    /// </summary>
    public int Length => 4 + 32 + Payload.Length + 32;
    ...
    byte[] ComputeHash()
    {
        ...
        byte[] combined = new byte[32 + Payload.Length];
        Array.Copy(Nonce, 0, combined, 0, 32);
        Array.Copy(Payload, 0, combined, 32, Payload.Length);

        cachedHash = Sha256.Hash(combined);
        return cachedHash;
    }
    ...
    public byte[] ToBytes()
    {
        ...
        uint size = (uint)(32 + Payload.Length + 32); // nonce + payload + hash
        ...
        Array.Copy(Nonce, 0, data, offset, 32);
        offset += 32;
    ...
    {
        AdnlPacket? packet = TryParse(data);
        if (packet == null)
