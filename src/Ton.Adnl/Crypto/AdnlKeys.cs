...
public sealed class AdnlKeys
{
    ...
    public AdnlKeys(byte[] peerPublicKey)
    {
        ...
        Array.Copy(seed, 0, _privateKey, 0, 32);
        Array.Copy(publicKey, 0, _privateKey, 32, 32);
    ...
    /// </summary>
    public byte[] PeerPublicKey => _peerPublicKey;
    ...
    public byte[] SharedSecret
    {
        get
        {
            if (_sharedSecret == null)
            {
                ...
                byte[] privateX25519 = ConvertEd25519PrivateToX25519(_privateKey);
                byte[] publicX25519 = ConvertEd25519PublicToX25519(_peerPublicKey);

                var x25519Private = new X25519PrivateKeyParameters(privateX25519, 0);
                var x25519Public = new X25519PublicKeyParameters(publicX25519, 0);

                _sharedSecret = new byte[32];
                x25519Private.GenerateSecret(x25519Public, _sharedSecret, 0);
            }
    ...
    /// </summary>
    private static byte[] ConvertEd25519PrivateToX25519(byte[] ed25519PrivateKey)
    {
    ...
    private static byte[] ConvertEd25519PublicToX25519(byte[] ed25519PublicKey)
    {
        ...
        byte[] uBytes = u.ToByteArray(isUnsigned: true, isBigEndian: false);
